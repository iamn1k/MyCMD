...
//	  Везде вместо вывода alert значения должны выводиться в созданную консоль!
    			...
    			function Information(){ // Объявление функции "Информация"
 		 			document.getElementById("String1").innerHTML = "Command line online!";// Вывод текста в строку
  					...
  					document.getElementById("String2").innerHTML = "To continue, enter the commands in the line below";// Вывод текста в строку
  					...
  					document.getElementById("String3").innerHTML = "Find out the list of commands below"; // Вывод текста в строку
  					...
  					document.getElementById("String4").innerHTML = "Date Created: August 2019";// Вывод текста в строку
					...
					if (event.keyCode == 13){ // Если клавиша это Enter
		    			...
		    			document.getElementById("Clear_cmd").onclick = function(e){
							...
							if(value_entered.length < 1){ // Проверка на пустую строку
								...
								document.getElementById("String1").innerHTML = "Enter at least something ...";// Предупреждение пользователя, что он ввел пустую строку
							}
                            						...
                            						if(value_entered =="!info"){ // Если введена команда !info
                            							...
                            							if(value_entered == "!getIP"){ // Если введена команда !getIP
                            								...
                            								if(value_entered == "!Vk"){// Если введенное значение равно !Vk
                            									Clear();// Очищение рабочей области								
                            										document.getElementById("String1").innerHTML = ("<a href=\"https://vk.com\" target=\"_blank\">Go to VK!</a>");// Добавляет ссылку для перехода в вк а также ссылка открывается в новом окне	
	                            										var style_string = document.getElementById("String1"); // Получение елемента по ID 
  																			style_string.style.color = "#ebe0df";// Установка цвета для команд на белый
  																			style_string.style.fontSize = "15px";// Размер шрифта
  																			style_string.style.fontWeight = "bolder";// Жирный шрифт
                            									...
                            									if(value_entered == "!Stack"){
                            										Clear();
                            											document.getElementById("String1").innerHTML = ("<a href=\"https://ru.stackoverflow.com/questions\" target=\"_blank\">Go to StackOverFlow!</a>");// Добавляет ссылку для перехода в вк а также ссылка открывается в новом окне	
	                            										var style_string = document.getElementById("String1"); // Получение елемента по ID 
  																			style_string.style.color = "#ebe0df";// Установка цвета для команд на белый
  																			style_string.style.fontSize = "15px";// Размер шрифта
  																			style_string.style.fontWeight = "bolder";// Жирный шрифт
                            										...
                            										if(value_entered == "!RussianVersion"){
                            											Clear();
                            											document.getElementById("String1").innerHTML = ("<a href=\"file:///D:/%D0%BC%D0%B0%D0%BC%D0%B8%D0%BD%20%D0%BF%D1%80%D0%BE%D0%B3%D1%80%D0%B0%D0%BC%D0%B8%D1%81%D1%82/MyCMD/MyCMD.html\">Russian Version</a>");
                            										}
                                                  if(value_entered == "!MusicVk"){
                                                    Clear();  
                                                    document.getElementById("String1").innerHTML = ("<a href=\"https://vk.com/music\" target=\"_blank\">Music Vk!</a>");
                                                  }
                            							
									}
										else{		 // Если введено обычное слово, а не команда
											Clear(); // Функция очищения рабочей области
											document.getElementById("String1").innerHTML = "Word " + value_entered + " not be a command!"; // Информирование пользователя об ошибке ввода
										}
				    			}
					}
